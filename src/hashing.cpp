...
const unsigned char offsets[] = {1, 5, 6, 11, 20, 22, 28, 33, 36, 41, 49, 58, 65, 74, 77, 79, 82, 90, 94, 100, 102, 108,
                                 112, 115, 118, 126, 129, 131, 138, 147, 152, 154};
...
template<class T>
bool updateHashForArray(T hasher, const nlohmann::json &array, bool isFirst) {
...
template<class T>
bool updateHashForChar(T hasher, const char *charToHash) {
#ifdef DEBUG_HASHING
    std::clog << "$s$" << std::endl;
#endif
...
template<class T>
bool updateHashForInteger(T hasher, const int integer) {
    char str[16];
...
template<class T>
bool updateHashForObject(T hasher, const nlohmann::json &object, bool isFirst) {
    ...
    for (auto &it : object.items()) {
        ...
        if (numberOfKeysSorted > 0) {
            ...
            if (key_str.size() >= MAX_KEY_SIZE)
                return false;

            while (isChar1BeforeChar2(key_str.c_str(), sortedKeys[i - 1]) && i > 0) {
                i--;
            }
            for (int j = numberOfKeysSorted; j > i; j--) {
                strcpy(sortedKeys[j], sortedKeys[j - 1]);
            }
            strcpy(sortedKeys[i], key_str.c_str());
        } else {
            strcpy(sortedKeys[0], key_str.c_str());
        }
        numberOfKeysSorted++;
    }
